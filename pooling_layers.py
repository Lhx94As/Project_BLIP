...
class SElayer_random(nn.Module):
    ...
    def forward(self, x, mask_mean=None, seq_weights=1):
        if mask_mean is not None:
            assert mask_mean.size() == x.size()
            x.masked_fill_(mask_mean, 1e-5)
...
class dimension_selection(nn.Module): #attentive squeeze and excitation
    def __init__(self, featdim=1024, reduction=16):
        super(dimension_selection, self).__init__()
        self.featdim = featdim
        self.factor = np.sqrt(featdim)
        self.lang_v = torch.nn.Parameter(torch.randn(featdim))
        self.squeeze_excitation = nn.Sequential(nn.Linear(featdim, featdim // reduction, bias=False),
                                                nn.ReLU(inplace=True),
                                                nn.Linear(featdim // reduction, featdim, bias=False),
                                                nn.Sigmoid())
    def forward(self, x, mask):
        v_ = self.lang_v.unsqueeze(0).expand(x.size(0), self.featdim).unsqueeze(2)
        # print(v_.size())
        similarity = F.relu(x.bmm(v_).squeeze(2))/ self.factor
        if mask is not None:
            assert  mask.size() == similarity.size()
            similarity.masked_fill_(mask, -1e9)
        attention_weght = F.softmax(similarity, dim=1)
        x_mean = torch.mul(attention_weght.unsqueeze(2), x).sum(dim=1) # weighted sum
        weight = self.squeeze_excitation(x_mean).unsqueeze(1)
        out = x * weight
        return out, weight.squeeze(1), x_mean
